...
module.exports = (pool) => {
  ...
  // — Helper para generar el bracket automáticamente
  async function generateBracket(torneoId) {
    // 1) Obtener todos los equipos inscritos
    ...
    // 4) Insertar partidos y actualizar estado
    await pool.query(
  ...
  // — Sólo admins pueden acceder a estas rutas
  ...
  router.post('/', async (req, res, next) => {
    ...
    try {
      await pool.query(
        `INSERT INTO torneos
           (nombre, descripcion, fecha, ubicacion_x, ubicacion_y, max_equipos, estado, creado_por)
         VALUES (?, ?, ?, ?, ?, ?, 'abierto', ?)`,
        [
          nombre,
          descripcion || null,
  ...
  router.post('/:id/join', async (req, res, next) => {
    ...
    try {
      ...
      // Generar bracket si justo alcanzamos el cupo
