...
module.exports = (pool) => {
  ...
  router.get("/new", (req, res) => {
    res.render("teams/new", { user: req.user, error: null });
  });
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO equipos (nombre, limite_miembros, premium, foto)
         VALUES (?, ?, ?, ?)`,
        [nombre, limite_miembros, premium === "on" ? 1 : 0, fotoPath]
      );
  ...
  router.get("/:id", async (req, res, next) => {
    try {
      const [equipo] = await pool.query("SELECT * FROM equipos WHERE id = ?", [req.params.id]);
      if (!equipo) return res.status(404).render("errors/404");
  ...
  router.get("/:id/edit", async (req, res, next) => {
    try {
      ...
      if (!equipo) return res.status(404).render("errors/404");
      res.render("teams/edit", { equipo, user: req.user });
    } catch (err) {
      next(err);
    }
  });
  ...
  router.put("/:id", upload.single('foto'), async (req, res, next) => {
    try {
      ...
      if (limite_miembros)    { fields.push("limite_miembros = ?");  params.push(limite_miembros); }
      if (premium != null)    { fields.push("premium = ?");          params.push(premium === "on" ? 1 : 0); }
